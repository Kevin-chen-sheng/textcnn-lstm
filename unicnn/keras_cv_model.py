...
def create_model(optimizer='adam',n_filter=100,kernel_size=8,mp_size=20,dense_num=200,activation_f='relu'):
    model=Sequential()
    model.add(Conv1D(filters=n_filter,kernel_size=kernel_size,activation=activation_f,input_shape=(40,20)))
...
def duliceshi(dataset,dataset1,label, labeltest):
    ...
    test_y = labeltest
    test_y = np_utils.to_categorical(test_y)
    early_stop = EarlyStopping(patience=10, verbose=1, monitor='val_acc')
    checkpoint = ModelCheckpoint('cv_model_' + 'mylstm', verbose=1, monitor='val_acc', save_best_only=True)
    model = create_model()
    # model.fit(train_x, train_y, validation_data=[test_x, test_y], epochs=30, batch_size=500,
    #           callbacks=[checkpoint, early_stop])
    model.fit(train_x, train_y, epochs=30, batch_size=200,
              callbacks=[checkpoint, early_stop])
    model = load_model('cv_model_' + 'mylstm')
